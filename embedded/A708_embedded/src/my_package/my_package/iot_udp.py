...
appliance = {
    "living_room": {  # 거실 (조명/에어컨/TV)
        "light": {"status": "OFF"},
        "air_conditioner": {"status": "OFF", "mode": "cool", "speed": "mid", "target": 23},
        # mode = cool, warm, dry
        # speed = high, mid, low
        "tv": {"status": "OFF"},
    },
    "inner_room": {  # 안방 (조명/에어컨/공기청정기)
        "light": {"status": "OFF"},
        "air_conditioner": {"status": "OFF", "mode": "cool", "speed": "mid"},
        "air_cleaner": {"status": "OFF", "mode": "high"},
        # mode = high, mid, low
    },
    "library": {  # 서재 (조명/공기청정기)
        "light": {"status": "OFF"},
        "air_cleaner": {"status": "OFF", },
    },
...
class iot_udp(Node):
    ...
    def device_control(self, device_name):
        goal_pose = PoseStamped()
        goal_pose.pose.position.x = device_pose[device_name][0]
        goal_pose.pose.position.y = device_pose[device_name][1]
        goal_pose.pose.orientation.w = 1.0
        self.goal_pub.publish(goal_pose)
        if self.is_status == True:
            while not (device_pose[device_name][0]-0.5 <= self.status_msg.twist.angular.x <= device_pose[device_name][0]+0.5) or not (device_pose[device_name][1]-0.5 <= self.status_msg.twist.angular.y <= device_pose[device_name][1]+0.5):
...
def robot_message(data):  # !!!chat_message를 robot_message로 변경했습니다!!!
    ...
    iot.device_control(dict2["device"])
